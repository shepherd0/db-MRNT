    ...
    with torch.no_grad():
        ...
        for i in range(args.test_times):
            ...
            for test_episode in range(args.test_episode):
                ...
                f123O = fpn_score1 + fpn_score2 + fpn_score3
                f123 = (fpn_score1 + fpn_score2 + fpn_score3) / 3.0
                predict_labels_f123 = f123 + scores_multi
                predict_labels = f123O + scores_multi
                # score_total = scores_multi
                _, predict_labels_f123 = torch.max(predict_labels_f123.data, 1)
                _, predict_labels = torch.max(predict_labels.data, 1)
                # _, score_total = torch.max(score_total.data, 1)
                ...
                # rewards_total = score_total.eq(local.to(device)).sum().item()
                ...
                #
                # test_accuracy_total = rewards_total / 1.0 / (args.way * args.query)
                # per_100_acc_total += test_accuracy_total
                # accuracies_total.append(test_accuracy_total)

                if (test_episode + 1) % 100 == 0:
                    print("test_accuracy :", per_100_acc / 100, "test_accuracy_f123 :", per_100_acc_f123 / 100)
                    # print("test_accuracy :", per_100_acc / 100)
                    per_100_acc = 0.0
...
def main():
    ...
    model.load_state_dict(torch.load("./models/check_point/" +
                                     "Episode202500-test_accuracy-0.579111111111111.pkl", map_location={'cuda:': 'cuda:' + str(args.gpu)}))
    print("load model ok!")
    model.to(device)
