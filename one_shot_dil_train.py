...
def train(model, task_generator):
    ...
    scheduler = StepLR(optim, step_size=80000, gamma=0.5)
    """
    """
    ...
    for train_episode in range(0, TRAIN_EPISODE):
        ...
        # 单独指导
        ditillation_loss_T_S1 = criterion_ditillation(F.softmax(fpn_score1 / T1, dim=1), F.softmax(scores_multi / T1, dim=1))
        lossT_S1 = lossf1 * alpha + ditillation_loss_T_S1*(1-alpha)

        ditillation_loss_T_S2 = criterion_ditillation(F.softmax(fpn_score2 / T2, dim=1), F.softmax(scores_multi / T2, dim=1))
        lossT_S2 = lossf2 * alpha + ditillation_loss_T_S2*(1-alpha)

        ditillation_loss_T_S3 = criterion_ditillation(F.softmax(fpn_score3 / T3, dim=1), F.softmax(scores_multi / T3, dim=1))
        lossT_S3 = lossf3 * alpha + ditillation_loss_T_S3*(1-alpha)

        loss_all = lossplus + lossT_S1 + lossT_S2 + lossT_S3
        ...
        # eval and choose the best modelsT
        if (train_episode + 1) % 5000 == 0:
            model.eval()
            ...
            with torch.no_grad():
                ...
                print("score_multi:", rewardsm, "score_21:", rewards0, "score_10:", rewards1, "score_15:", rewards2,
                      "score_17: ", rewards3, "score_19:", rewards4)
                print("score_multi:", rewardsm, "score_21:", rewards0, "score_10:", rewards1, "score_15:", rewards2,
                      "score_17:", rewards3, "score_19:", rewards4, file=F_txt)
...
"""
name: miniImageNet数据集上的5-way 1-shot训练
description: 分别蒸馏三个层次
"""