...
class Conv64(nn.Module):
    ...
    def forward(self, x):
        out = self.layer1(x)  # 42
        out1 = self.layer2(out)  # 42
...
class FPNRelationNet(nn.Module):
    ...
    def forward(self, x):  # (375,64,21,21)
        ...
        out = self.layer2(out)  # (375,64,9,9),(4,4),(1,1)
        out = self.avg(out)  # (375,64,1,1)
...
class MsCRN(nn.Module):
    def __init__(self, num_class, num_support, num_query, embedding_net="Conv64"):
        ...
        self.relation2 = FPNRelationNet()
        self.relation3 = FPNRelationNet()
    ...
    def forward(self, support_x, query_x, class_num, class_query, feature_dim):
        """
            ...
            sf1=(25,64,10,10)  sf2=(25,64,19,19)  sf3=(25,64,15,15)  sf4=(25,64,17,17)
            qf1=(75,64,10,10) qf2=(75,64,19,19) qf3=(75,64,15,15) qf4=(75,64,17,17)
        """
        # support_x=(5,3,84,84)  query_x=(75,3,84,84)

        # feature_encoding
        # (75,64,42,42)
        support_feature0, support_features = self.feature_encoder(support_x)  # (21)(25,64,21,21)
        query_feature0, query_features = self.feature_encoder(query_x)  # (21)(75,64,21,21)
        ...
        # multy scale feature prototypes
        s_fea_s1 = s_fea_s1.view(class_num, 5, feature_dim, 10, 10).mean(1)  # (5,64,10,10) ***
        s_fea_s2 = s_fea_s2.view(class_num, 5, feature_dim, 19, 19).mean(1)  # (5,64,19,19)
        s_fea_s3 = s_fea_s3.view(class_num, 5, feature_dim, 15, 15).mean(1)  # (5,64,15,15)
        s_fea_s4 = s_fea_s4.view(class_num, 5, feature_dim, 17, 17).mean(1)  # (5,64,17,17)
        support_features = support_features.view(class_num, 5, feature_dim, 21, 21).mean(1)  # (5,64,21,21)***
        support_feature0 = support_feature0.view(class_num, 5, feature_dim, 42, 42).mean(1)  # (5,64,42,42)***
        ...
        fpn_score1 = self.relation1(sq_cat)  # score, 42

        sq_cat = torch.cat((support_features, query_features), 1)  # (375,128,21,21)
        fpn_score2 = self.relation2(sq_cat)  # 21

        sq_cat = torch.cat((s_fea_s1, q_fea_s1), 1)  # (375,128,10,10)
        fpn_score3 = self.relation3(sq_cat)  # 10

        # multy-scale
        s_fea_s2 = s_fea_s2.unsqueeze(0).repeat(class_num * class_query, 1, 1, 1, 1)  # (75,5,64,19,19)
        q_fea_s2 = q_fea_s2.unsqueeze(1).repeat(1, class_num, 1, 1, 1)  # (75,5,64,19,19)
...
"""
network_final3:
description:network_fpn大改，将9的尺寸改为10.加了一个padding，将42的尺度由最底层卷积改为后一层卷积（maxpool延后一层）
            将19尺寸改为19，空洞卷积5-2，
            先计算FPN，再计算多尺度，没有FPN关系传递（RN2），没有上采样
            基础模型2.0版本,多尺度的原型
"""
